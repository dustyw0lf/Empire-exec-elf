# Credit to Vincent Berg the for original code
...
def exec_elf(elf, args):
    # sanity check where the script is being run
    if os.name != "posix" or os.uname()[0].lower() != "linux":
        logging.error("This script only works on Linux")
        sys.exit(1)

    with open(elf, "rb") as binfd:
        executor = ELFExecutor(binfd, elf)
...
# exec_elf('{{ elf }}','{{ args }}')
exec_elf('/usr/bin/echo','it works!')
